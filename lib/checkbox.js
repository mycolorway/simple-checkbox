...
Checkbox = (function(_super) {
  ...
  function Checkbox() {
    this._endAnimate = __bind(this._endAnimate, this);
  ...
  Checkbox.prototype._render = function() {
    ...
    if (this.el.length < 1) {
      ...
      this.el.find('.checkbox-container').css({
        border: 0.12 * this.opts.size + 'px solid'
      });
      this.el.find('.checkbox-tick').css({
        'border-right': 0.12 * this.opts.size + 'px solid',
        'border-bottom': 0.12 * this.opts.size + 'px solid'
      });
    }
    this.ripple = this.el.find('.checkbox-ripple');
  ...
  Checkbox.prototype._startAnimate = function() {
    if (this.el.hasClass('animation-start animation-end')) {
      return;
    }
    this.el.addClass('animation-start');
    return this.ripple.on('transitionend webkitTransitionEnd', (function(_this) {
      return function(e) {
        if (e.originalEvent.propertyName === 'transform') {
          return;
        }
        _this.ripple.off('transitionend webkitTransitionEnd');
        if (_this.el.hasClass('pressed')) {
          _this.el.one('mouseup', function() {
            return _this._endAnimate();
          });
          return;
        }
        return _this._endAnimate();
  ...
  Checkbox.prototype._endAnimate = function() {
    this.el.addClass('animation-end');
    return this.ripple.on('transitionend webkitTransitionEnd', (function(_this) {
      return function(e) {
        if (e.originalEvent.propertyName === 'transform') {
          return;
        ...
        _this.ripple.off('transitionend webkitTransitionEnd');
        return _this.el.removeClass('animation-start animation-end');
      };
    })(this));
  };
